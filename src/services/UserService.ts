...
export class UserService {
   ...
   public static async init() {
      ...
      Store.instance.onUsernameChange((username) => UserService.onUsernameChange(username));
   }
   ...
   public static async onUsernameChange(username: string | null): Promise<void> {
      if (username) {
         ...
         Utils.updateUninstallHook()
            .then((result) => {
               console.log("[UserService -> onUsernameChange] setUninstallHook set to ", result);
            })
            .catch((e) => {
               console.error("[UserService -> onUsernameChange] setUninstallHook error", e);
            });
      }
   }
}
