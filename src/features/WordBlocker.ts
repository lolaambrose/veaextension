...
export class WordBlocker {
    ...
    constructor() {
        ...
        Store.instance
            ...
            .catch((error) => {
                console.log(
                    "WordBlocker -> init error while fetching forbiddenWords: ",
                    error.message,
                );
            });

        Store.instance.onForbiddenWordsChange((words) =>
            this.onForbiddenWordsChange(words),
        );
    }
        ...
        if (forbiddenWords) {
            ...
            forbiddenWords.forEach((phrase) => {
                ...
                if (textContent.match(regex)) {
                    console.log(
                        `[WordBlocker -> checkAndBlockInput] forbidden phrase found: ${phrase}`,
                    );
                    isBlocking = true;
                }
            });
        ...
        if (forbiddenWords) {
            ...
            if (allWordsFound.length > 0 && username && loginTime) {
                ApiClient.reportBanword(allWordsFound[0], username, loginTime)
                    .then(() => {
                        console.log(
                            "[WordBlocker -> checkAndBlockInput] reportBanword success",
                        );
                    })
                    .catch((error) => {
                        console.log(
    ...
    public async checkNode(node: Node): Promise<void> {
        if (!this._forbiddenWords) {
            console.log(
                "[WordBlocker -> checkNode] awaiting forbiddenWords for load",
            );
        ...
        if (textArea && !this._listenedNodes.includes(textArea)) {
            ...
            console.log(
                "[WordBlocker -> checkNode] listener added for " +
